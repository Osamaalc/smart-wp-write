...
class CohereProvider(LLMInterface):
    ...
    def generate_text(self, prompt: str, chat_hostory: list, max_output_tokens: int, temperature: float = None):
        if not self.client:
    ...
    #         texts=self.process_text(text),  # ✅ إرسال النص كسلسلة نصية مباشرةً
    ...
    def embed_text(self, text: str, document_type: str = None):
        ...
        # ✅ التحقق من حجم المتجه قبل الإرجاع
        embedding = response.embeddings[0]
        if len(embedding) != self.embedding_size:
            self.logger.warning(
                f"⚠️ Invalid embedding size: {len(embedding)} (expected {self.embedding_size})"
            )
            return None
