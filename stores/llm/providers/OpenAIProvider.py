...
class OpenAIProvider(LLMInterface):
    def __init__(self, api_key: str, api_url: str = None,
        ...
        self.client = OpenAI(
            ...
            base_url=self.api_url if self.api_url and len(self.api_url) else  None,  # استخدام api_base بدلاً من api_url
        )
    ...
    def generate_text(self, prompt: str, chat_history: list, max_output_tokens: int = None, temperature: float = None):
        ...
        rosponse = self.client.chat.completions.create(
        ...
        if not rosponse or not rosponse.choices or len(rosponse.choices) == 0 or not rosponse.choices[0].message:
            self.logger.error("Error while generating text using OpenAI")
            return None
        return rosponse.choices[0].message.content
