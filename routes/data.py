...
# إعداد logger لتسجيل الأخطاء
logger = logging.getLogger('uvicorn.error')

# إنشاء router لواجهات البيانات
data_router = APIRouter(
    prefix="/api/v1/data",  # بادئة المسار
    tags=["api_v1", "data"]  # وسم للتوثيق
)
...
async def upload_data(request:Request,project_id: str, file: UploadFile, app_settings: Settings = Depends(get_settings)):
    """
    واجهة لتحميل الملفات إلى مجلد مشروع محدد.
    """
    # إنشاء كائن من DataController للتحقق من صحة الملف
    ...
    # إذا كان الملف غير صالح، يتم إرجاع رسالة خطأ
    if not is_valid:
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content={"signal": result_signal}  # استخدام قاموس للإشارة
        )
    ...
    try:
        # تحميل الملف إلى المسار المحدد
        async with aiofiles.open(file_path, 'wb') as f:
            while chunk := await file.read(app_settings.FILE_DEFAULT_CHUNK_SIZE):
                await f.write(chunk)

    except Exception as e:
        # تسجيل الخطأ في حالة فشل التحميل
        logger.error(f"Error while uploading file: {e}")
        return JSONResponse(
            content={"signal": ResponseSignal.FILE_UPLOAD_FAILED.value}
        )
    #store the assets into dataset
    ...
    # إرجاع رسالة نجاح في حالة نجاح التحميل
    return JSONResponse(
    ...
    for asset_id,file_id in project_files_ids.items():
        ...
        file_chunks_records = [
            DataChunk(
                ...
                chunk_metadata=chunk.metadata,  # تأكد من تعريف chunk_size
                chunk_order=i + 1,
                chunk_project_id=project.id, # تأكد من تعريف Project و Project._id
